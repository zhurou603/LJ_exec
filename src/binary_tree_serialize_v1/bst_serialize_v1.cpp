...
string serialize(shared_ptr<TreeNode> root){
  ...
  count_node--;
  if((count_node & 0x3f) != 0) stat.push_back(stat_bits);
...
shared_ptr<Tpair> recursive_deserialize(const string& stat_string,
	...
	//拿当前结点的stat
  bool current_bit = (stat_string[(start/64)*8 + (7 - (start%64)/8)] >> (start % 64)%8) & 0x1;
  //为空返回
  if(!current_bit){
  ...
  //递归左子树
  shared_ptr<Tpair> p1 = recursive_deserialize(stat_string, value, value_index, start + 1);
  node->left = p1->node;
  
  //递归右子树
  shared_ptr<Tpair> p2 = recursive_deserialize(stat_string, value, value_index, p1->startIndex);
  node->right = p2->node;
