...
shared_ptr<Node> InterNode::get_first_child() const{
...
shared_ptr<Node> InterNode::find(Key key){
...
void InterNode::fill_new_root(shared_ptr<InterNode> old_node, Key key, shared_ptr<InterNode> new_node){
  keys_.push_back(make_pair(DUMMY_KEY, old_node));
  keys_.push_back(make_pair(key, new_node));
}

int InterNode::insert_after_old(shared_ptr<Node> old_node, Key key, shared_ptr<Node> new_node){
  auto finding = keys_.begin();
...
void InterNode::move_half_to(shared_ptr<InterNode> receiver){
  //先拷贝，后删除
...
void InterNode::copy_half_from(const vector<Pair>& origin_keys){
  ...
  for(int i = min_size()+1; i < origin_keys.size(); i++){
    //移动前先把parent信息更改为接受者
    origin_keys[i].second->set_parent((shared_ptr<InterNode>)this);
...
void InterNode::append_child_to_queue(queue<shared_ptr<Node>>* q){
  auto t = keys_.begin();
  while(t != keys_.end()){
    q->push(t->second);
    t++;
...
shared_ptr<Node> InterNode::cut_first_child(){
  shared_ptr<Node> first = get_first_child();
  keys_.pop_back();
  return first;
}

int InterNode::search_node_by_pointer(shared_ptr<Node> node){
...
shared_ptr<Node> InterNode::get_pos_node(int pos){
...
/*void InterNode::move_all_to(shared_ptr<InterNode> receiver, int pos){
  //更改dummy为合并后的key
  keys_.begin()->first = static_cast<shared_ptr<InterNode>>(get_parent())->get_pos_key(pos);
  //copy
...
void InterNode::move_first_to_end_of(shared_ptr<InterNode> node){
  node->copy_first_from(keys_.front());
  keys_.erase(keys_.begin());
  //删除后需要更新上一层的key
  static_cast<shared_ptr<InterNode>>(parent_)->set_key_pos(1, keys_.front().first); 
}
...
void InterNode::move_last_to_head_of(shared_ptr<InterNode> node, int node_pos){
  node->copy_last_from(keys_.back(),node_pos);
  keys_.pop_back();
}

void InterNode::copy_last_from(Pair last, int node_pos){
    keys_.front().first = static_cast<shared_ptr<InterNode>>(parent_)->get_pos_key(node_pos);
    keys_.insert(keys_.begin(), last);
    keys_.front().first = DUMMY_KEY;
    keys_.front().second->set_parent(this);
    static_cast<shared_ptr<InterNode>>(parent_)->set_key_pos(node_pos, keys_.front().first);
}*/
